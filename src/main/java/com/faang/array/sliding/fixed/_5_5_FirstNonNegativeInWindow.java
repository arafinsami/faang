...
/*
First Non-Negative Number in Every Window of Fixed Size K
...
1. Build / slide the window for each index:
      ...
      ii) Check whether the current element is non-negative
          {array[index] >= 0}

      iii) If non-negative, add its index to the back of the deque
...
2. Process the window:
      ...
      ii) If deque is not empty:
          - The index at the front represents the
            first non-negative number in the current window
          - Add {array[deque.peekFirst()]} to the result

      iii) If deque is empty:
           - There is no non-negative number in the current window
           - Add 0 to the result
*/
